    ...
    try:
        async for raw_message in websocket:
            ...
            if msg_role == "esp8266" and role is None:
                ...
                await websocket.send(json.dumps({"staus":"registered", "role": "esp8266"}))
                continue
            ...
            if msg_role == "phone" and role is None:
                ...
                await websocket.send(json.dumps({"staus":"registered", "role": "phone"}))

            if role == "phone" and "angle" in data:
                esp = clients["esp8266"]
                if esp is not None:
                    awit esp.send(json.dumps({"angle": data["angle"]}))
                else:
